...
const isBuilding=x=>x?.station==='BUILD'&&!x.built;
...
function optimisedPlacements(baseP,plan){
  ...
  for(const unit of units){
    ...
    if(cycleStatus.kind==='unused'&&!isIconic(d)){
      ...
      if(spared.includes(key)&&['LOUNGE','COMPANION','BUILD'].some(station=>free(station)>=0)){
        keptByHand.set(key,'Kept by you · you chose not to sell this one');
        candidates.push({unit,fallbacks:['LOUNGE','COMPANION'],old:current.get(key),betterStorageOpen:false,kept:false});
        continue;
      }
      ...
      if(gaps.length&&!droidexKeepers.has(unit.name)&&['LOUNGE','COMPANION'].some(station=>free(station)>=0)){
...
function modernButtonIcon(button){const text=button.textContent.toLowerCase();if(text.includes('map'))return'map';if(text.includes('health'))return'health';if(text.includes('scrap'))return'scrap';if(text.includes('chip'))return'chips';if(text.includes('droid calc')||text.includes('replacement'))return'calc';if(text.includes('group outlook')||text.includes('outlook'))return'outlook';if(text.includes('import')||text.includes('export'))return'transfer';if(text.includes('add'))return'add';return'detail'}
function modernBaseSettings(){
  ...
  deck.querySelector('#toggleCommandOptimise').onclick=e=>{const panel=deck.querySelector('.modern-optimise-settings'),open=panel.hidden;panel.hidden=!open;localStorage.setItem('droid-archive-optimise-settings-open',open?'1':'0');e.currentTarget.setAttribute('aria-expanded',String(open));e.currentTarget.querySelector(':scope>b').textContent=open?'−':'+'};
  ...
  deck.querySelector('#commandOptimiseFreeBuild').onchange=e=>{state.optimiseFreeBuild=e.target.checked;rerender()};
...
Promise.all(['data/droids.json','data/rebirth-cycles/index.json','data/image-manifest.json','data/nova-shop.json','data/cantina-shop.json','data/stellar.json'].map(loadJson)).then(async([d,cycleIndex,i,novaShop,cantinaShop,stellarStats])=>{if(!Array.isArray(cycleIndex.cycles)||!cycleIndex.cycles.length)throw Error('No Super Rebirth cycles are configured.');const [cycles,events]=await Promise.all([Promise.all(cycleIndex.cycles.map(file=>loadJson(`data/rebirth-cycles/${file}`))),loadEvents()]);state.droids=applyStellarData(d,stellarStats);state.rebirths=Object.fromEntries(cycles.map((cycle,index)=>[index,cycle]));state.images=i;state.novaShop=novaShop;state.cantinaShop=cantinaShop;state.events=events;normalizeLoadedDroidNames();syncCantinaPackUpgrades();if(!Object.hasOwn(state.rebirths,String(state.cycle)))state.cycle=0;autoPurchaseEligibleSlots();saveLocal();attachModalBehaviour();route();loadPatchNotes().then(notes=>{state.patchNotes=notes;showPatchNotesOnce()});loadSupabaseConfig().then(()=>initSupabaseSafe()).then(()=>renderCloudHeader())}).catch(e=>{app.innerHTML=`<h1>Archive unavailable</h1><p>${e.message}</p>`});
